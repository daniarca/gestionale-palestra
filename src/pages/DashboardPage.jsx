...
function DashboardPage({ iscritti, loading, gruppi, pagamenti }) {
  ...
  const stats = useMemo(() => {
    ...
    const scadenzeDaVisualizzare = [
        ...certificatiScaduti.map((i) => ({ ...i, tipoScadenza: "Certificato Medico", scadenzaData: moment(i.certificatoMedico.scadenza), isScaduto: true })),
        ...certificatiInScadenza.map((i) => ({ ...i, tipoScadenza: "Certificato Medico", scadenzaData: moment(i.certificatoMedico.scadenza), isScaduto: false })),
        ...abbonamentiScaduti.map((i) => ({ ...i, tipoScadenza: "Abbonamento", scadenzaData: moment(i.abbonamento.scadenza), isScaduto: true })),
  ...
  return (
    <Box>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: 2, border: `1px solid ${theme.palette.divider}`, backgroundColor: 'background.paper' }}>
        <Typography variant="body1" sx={{ fontWeight: "bold", mb: 1, color: theme.palette.text.secondary }}>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} md={4}>
            <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 2 }}>Anagrafica & Corsi</Typography>
            <Paper sx={detailCardStyle}> {/* Contenitore Principale per Anagrafica & Corsi */}
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                        <Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: `1px solid ${theme.palette.divider}`, backgroundColor: 'background.default', textAlign: 'center', mt: 1 }}>
                            <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bold' }}>Stato Certificati</Typography>
                            <Stack direction="row" spacing={1} justifyContent="center" flexWrap="wrap">
                                <Chip label={`OK: ${stats.certificatiOKCount}`} icon={<CheckCircleIcon />} color="success" size="small" />
                                <Chip label={`In Scadenza: ${stats.certificatiInScadenzaCount}`} icon={<WarningIcon />} color="warning" size="small" />
                                <Chip label={`Scaduti: ${stats.certificatiScadutiCount}`} icon={<WarningIcon />} color="error" size="small" />
                            </Stack>
                        </Paper>
                    </Grid>
                </Grid>
        ...
        <Grid item xs={12} md={4}>
            <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 2 }}>Riepilogo Cassa</Typography>
            <Paper sx={detailCardStyle}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 1, color: theme.palette.success.main }}>
                    <AttachMoneyIcon sx={{ fontSize: '1.2rem' }} />
                    <Typography variant="body1" sx={{ ml: 1, fontWeight: 'medium' }}>Incasso Totale Complessivo</Typography>
                </Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                    ...
                    <Typography variant="body1" sx={{ fontWeight: 'bold', color: theme.palette.error.main }}>{stats.abbonamentiScadutiCount}</Typography>
                </Box>
                <Button 
                    component={RouterLink}
                    to="/report" 
        ...
        <Grid item xs={12} md={4}>
            <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 2 }}>Orari di Oggi</Typography>
            <Paper sx={detailCardStyle}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    <CalendarTodayIcon color="secondary" />
                    <Typography variant="h6" sx={{ ml: 1, fontWeight: 'bold' }}>{giorni[new Date().getDay()]}</Typography>
                </Box>
                <Divider sx={{ mb: 2 }} />
                ...
                {stats.orariDiOggi.length > 0 ? (
                    ...
                    {stats.orariDiOggi.map((gruppo) => (
                        ...
                        <ListItemText
                            ...
                            secondary={`Allenatore: ${gruppo.staffNome || 'N/D'} | Sede: ${gruppo.sede || 'N/D'}`}
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={6}>
            <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 2 }}>Scadenze da Controllare</Typography>
            <Paper sx={{...detailCardStyle, minHeight: { xs: 'auto', md: '450px' }}}> {/* Aumenta l'altezza delle liste per bilanciare */}
                        ...
                        <React.Fragment key={item.id + item.tipoScadenza}>
                            ...
                            >
                                <ListItemText
                                    ...
                                    secondary={`Scadenza ${item.tipoScadenza}: ${item.scadenzaData.format("DD/MM/YYYY")}`}
                            ...
                            {index < stats.scadenzeDaVisualizzare.length - 1 && <Divider />}
                        </React.Fragment>
                    ))}
                    </List>
                ) : (
        ...
        <Grid item xs={12} md={6}>
            <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 2 }}>Ultimi Pagamenti Registrati</Typography>
            <Paper sx={{...detailCardStyle, minHeight: { xs: 'auto', md: '450px' }}}> {/* Aumenta l'altezza delle liste per bilanciare */}
                <Box sx={{ maxHeight: 400, overflow: "auto" }}>
                        ...
                        {stats.pagamentiRecenti.map((p, index) => (
                            <React.Fragment key={index}>
                                <ListItem disableGutters>
                                    <ListItemText
                                        primary={<Typography component="span" sx={{ fontWeight: 'bold' }}>{p.iscrittoNome}</Typography>}
                                        secondary={p.dataPagamento}
                                    />
                                    <Box sx={{ textAlign: 'right' }}>
                                        <Chip label={`€${p.cifra.toFixed(2)}`} size="small" color="success" sx={{ mr: 1, fontWeight: 'bold' }} />
